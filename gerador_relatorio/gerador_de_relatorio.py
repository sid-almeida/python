...
def main():
    """Gerador de Relatório Automatizado."""

    menu = ["Home", "Pandas Profile", "Sweetviz", "Sobre"]
    choice = st.sidebar.selectbox("Menu", menu)

    if choice == "Pandas Profile":
        st.subheader("EDA Automatizado com Pandas Profile")
        ...
        st.subheader("EDA Automatizado com Sweetviz")
        data_file = st.file_uploader("Upload CSV", type=['csv'])
        if data_file is not None:
            df = pd.read_csv(data_file)
            st.dataframe(df.head())
        ...
        # components.iframe('https://github.com/sid-almeida/python')
